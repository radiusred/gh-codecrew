...
2. **Check your CLI against the hub's protocol** before any verb but
   `gh codecrew version` and `gh codecrew identity token`, which read no
   pointer and change nothing. `gh codecrew version` prints the protocol
   the binary implements; it must be the same major as the `codecrew:`
   field of the `.codecrew/config.yml` you read in step 1 — this repo is
   the hub, so that is the floor — and a minor at least its own: the CLI
   checks the major alone. If the binary falls short and you install the
