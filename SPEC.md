...
implicitly (§6). A label that already exists is **never** restyled —
colour and description both — because nothing can tell a colour the
project chose from one GitHub generated. These are defaults, not
enforcement: only the names are protocol.
