...
| `codecrew identity new <role> --name <app>` | Mints the role's App identity via the GitHub App manifest flow: generates a manifest with the role's minimal permission set, hands the operator a one-click loopback URL, stores the returned private key locally, and prints the remaining manual steps (install — per-account — and route). Webhooks off by default; `--with-webhook` opts in to protocol-traffic event delivery for platform receivers (§9). |
