...
verbs:
  ...
  migrate [--dry-run]                        move a protocol 1.x repo to the 2.0 layout: one local
                                             commit, never pushed, and the cc: labels restyled to the
                                             defaults (--dry-run: every step, nothing written)
  ...
  roles diff <role>|.codecrew/AGENTS.md      local contract, or agents file, vs the one embedded in the CLI
  roles show <role> [--latest]               the contract a session loads (with local extensions), or the embedded one
  roles sync [<role>...] [--dry-run]         write the embedded contracts and .codecrew/AGENTS.md where absent or a release's
           [.codecrew/AGENTS.md]             text (a spoke: the agents file alone; a role list skips it, its path names it):
